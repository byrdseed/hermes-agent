...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # A slash command can be the first event in a fresh session. In that
        # case there is no resident agent and the newly-created SessionDB row
        # has no billing metadata yet. Fall back to the configured gateway
        # model so `/usage` still queries the active provider instead of
        # dropping through to the unrelated Nous credits block.
        if not provider:
            try:
