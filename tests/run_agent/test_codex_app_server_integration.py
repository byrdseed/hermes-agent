...
class TestSessionRetirementOnRunAgent:
    ...
    def test_gateway_restart_loads_and_updates_durable_thread(
        ...
        """A fresh AIAgent after a process restart resumes the thread recorded
        on the Hermes session, then advances the durable binding."""
    ...
    def test_logical_session_boundary_never_reuses_prior_thread(
        ...
        """CLI-style session_id mutation cannot leak the old Codex thread."""
        resume_ids = []
        closes = {"count": 0}
